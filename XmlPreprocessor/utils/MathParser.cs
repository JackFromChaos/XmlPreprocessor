...
namespace dio.utils
{
	...
	{
		public delegate string dCustomVars(string name);
		public dCustomVars CustomVars;
		/// <summary>
		...
		public MathParser(bool loadPreDefinedFunctions = true, bool loadPreDefinedOperators = true, bool loadPreDefinedVariables = true)
		{
			...
			if (loadPreDefinedFunctions)
			{
				...
				LocalFunctions.Add("int", delegate (decimal[] a) { return (int)a[0]; });
				LocalFunctions.Add("round", delegate (decimal[] a) { return (int)Math.Round(a[0]); });
				LocalFunctions.Add("abs", delegate (decimal[] a) { return Math.Abs(a[0]); });
		...
		public delegate decimal func1(decimal a, decimal b);
		public delegate decimal func2(decimal[] a);
		...
		private Dictionary<string, func1> _operatorAction = new Dictionary<string, func1>();
		/// <summary>
		/// When adding a variable in the OperatorList property, you need to assign how that operator should work.
		/// </summary>
		public Dictionary<string, func1> OperatorAction
		{
			get { return _operatorAction; }
			set { _operatorAction = value; }
		}
		private Dictionary<string, func2> _localFunctions = new Dictionary<string, func2>();
		/// <summary>
		/// All functions that you want to define should be inside this property.
		/// </summary>
		public Dictionary<string, func2> LocalFunctions
		{
		...
		private decimal MathParserLogic(List<string> tokens)
		{
			...
			{
				if (LocalVariables.ContainsKey(tokens[i]))
				{
					tokens[i] = LocalVariables[tokens[i]].ToString(CultureInfo);
				}
				if (CustomVars != null)
				{
					string s = CustomVars(tokens[i]);
					if (s != null)
						tokens[i] = s;
				}
			}
			while (tokens.IndexOf("(") != -1)
			{
				...
				if (LocalFunctions.ContainsKey(functioName))
				{
